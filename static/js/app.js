function buildData() {
  ...
  d3.json(endpoint).then(function(data){
    ...
    // coordinateDataContent.append('h1').text("United Airlines US Hub Coordinates");
    data = data.coordinates;
    for (let i = 0; i < data.length; i++) {
      // coordinateData[i] is each inner list -> [key, value]
      // create a text string: 'key: value'
      let string = data[i][2] + ", " + data[i][3] + ", [" + data[i][0] + ': ' + data[i][1] + "]";
      // create a paragraph or text element to contain this string
      coordinateDataContent.append('p').text(string).classed("card-text",true);
